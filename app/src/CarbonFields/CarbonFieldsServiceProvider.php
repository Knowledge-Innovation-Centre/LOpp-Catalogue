	...
	protected function registerThemeOptions()
	{
		...
		$parentContainer = Container::make('theme_options', __('LOC Options', 'learning_opportunities_catalogue'))
			...
			->add_fields([
				...
				Field::make('text', 'meilisearch_index_key', __('Meilisearch index key'))
				...
				Field::make('checkbox', 'enable_lmm', __('Enable learning maturity models'))->set_option_value('yes'),
				Field::make('checkbox', 'hide_rows_no_data', __('Hide rows with no data'))->set_option_value('yes'),
				Field::make('checkbox', 'hide_table_on_catalogue_single_page', __('Hide table on catalogue single page'))
				...
				Field::make('complex', 'languages' . self::crb_get_i18n_suffix(), __('Languages'))
					...
					->add_fields([
						...
						Field::make('select', 'wpml_code', __('WPML language'))
							->set_options($languagesOptions)
							->set_width(33),
					]),
		...
		foreach ($fields as $fieldsForThemeOptions) {
			...
			$fields_for_theme_options[] = Field::make('hidden', ($post_type . '_hidden' . rand()), __('Filter'))
				->set_width(30);
			$fields_for_theme_options[] = Field::make('hidden', ($post_type . '_hidden' . rand()), __('Label'))
				->set_width(30);
			...
			$fields_for_theme_options[] = Field::make('hidden', ($post_type . '_hidden' . rand()), __('Required'))
				->set_width(8);
			$fields_for_theme_options[] = Field::make('hidden', ($post_type . '_hidden' . rand()), __('Is url'))
				->set_width(8);
		...
		$container->add_tab(__('Additional fields'), [
			Field::make('complex', 'loc_option_catalogue_fields', __('Catalog item fields'))
				...
				->add_fields([
					Field::make('hidden', 'hidden1', __('Title'))->set_width(20),
					Field::make('hidden', 'hidden2', __('Slug'))->set_width(20),
					Field::make('hidden', 'hidden3', __('Type'))->set_width(8),
					Field::make('hidden', 'hidden4', __('Search weight'))->set_width(8),
					Field::make('hidden', 'hidden5', __('Filter'))->set_width(8),
					Field::make('hidden', 'hidden6', __('Width in editor'))->set_width(8),
					Field::make('hidden', 'hidden7', __('Visible in website (frontend) table'))->set_width(6),
					Field::make('hidden', 'hidden8', __('Visible in catalogue list (frontend)'))->set_width(6),
					Field::make('hidden', 'hidden9', __('Required'))->set_width(6),
					Field::make('hidden', 'hidden10', __('Is url'))->set_width(6),
					...
					Field::make('checkbox', 'required', '')->set_option_value('yes')->set_width(6),
					Field::make('checkbox', 'is_url', '')->set_option_value('yes')->set_width(6),
