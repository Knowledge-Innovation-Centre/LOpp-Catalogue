	...
	class CatalogueImporter
	{
		...
		public function import()
		{
			...
			try {
				...
				$query = "SELECT option_name, option_value FROM "
						 . $wpdb->prefix
						 . "options WHERE option_name LIKE '%_required'";
		...
		public function import_vocabularies()
		{
			...
			try {
				...
				if ($xml->code) {
					foreach ($xml->code as $code) {
						if ((string)$code->attributes()->value) {
							$items[] = [
								'name' => trim((string)$code->attributes()->value)
										  . ' - '
										  . trim((string)$code->attributes()->description),
							];
						}
					}
				}
				...
				carbon_set_theme_option($import_option_type
										. CarbonFieldsServiceProvider::crb_get_i18n_suffix(), $items);
		...
		private function update_post_field($post_id, $xmlData, $field)
		{
			...
			if ((! $value || $value == "")
				&& isset($this->requiredFields[$slug])
				&& $this->requiredFields[$slug]->option_value == 'yes') {
			...
			if (is_array($value)) {
				...
				foreach ($value as $valueItem) {
					$associationValues[] = [
						'value'   => 'post:' . LearningOutcome::POST_TYPE . ':' . $valueItem,
						'id'      => $valueItem,
						'type'    => 'post',
						'subtype' => LearningOutcome::POST_TYPE,
					];
				}
				carbon_set_post_meta($post_id, $field['slug'], $associationValues);
			} else {
