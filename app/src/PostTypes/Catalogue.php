	...
	class Catalogue {
		...
		function add_content_after( $content ) {
			...
			if ( is_single() && $post->post_type == $this->post_type ) {
				...
				$content .= '<p>'  . get_the_excerpt($post) . '</p>';
				$content .= '
				...
				<table class="loc-other-information tw-hidden">
					<tbody>';
				...
				<div class="w-full text-center"><button class="loc-show-other-information">' . __( 'Show more information' ) . '</button><button class="loc-hide-other-information tw-hidden" >' . __( 'Hide more information' ) . '</button></div>
				';
			...
			if (!is_array($value)) { return $content; }
				foreach ( $value as $valueItem ) {
					...
					if (isset($valueItem['subtype']) && $valueItem['subtype'] == LearningOutcome::POST_TYPE) {
						foreach ( LearningOutcomeFields::get_general_fields() as $field ) :
					...
					if (isset($valueItem['subtype']) && $valueItem['subtype'] == DimensionSubsetItem::POST_TYPE) {
						...
						$content  .= $this->getTableRow( __('Dimension subset item'), get_the_title($valueItem['id']));
						$content  .= $this->getTableRow( __('Dimension subset item description'), get_the_content($valueItem['id']));
						$content  .= $this->getTableRowCarbon($valueItem['id'], 'level', __('Level'));
						$content  .= $this->getTableRowCarbon($valueItem['id'], 'type', __('Type'));
					}
					if (isset($valueItem['subtype']) && $valueItem['subtype'] == DimensionSubset::POST_TYPE) {
						...
						$content  .= $this->getTableRow( __('Dimension subset description'), get_the_content($valueItem['id']));
					...
					if (isset($valueItem['subtype']) && $valueItem['subtype'] == Dimension::POST_TYPE) {
						...
						$content  .= $this->getTableRow( __('Dimension description'), get_the_content($valueItem['id']));
					}
				}
